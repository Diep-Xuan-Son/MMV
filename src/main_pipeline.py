...
class VideoMaketing(object):
    def __init__(self, 
        ...
        # init workers
        self.hlw = HighlightWorker(model_hl=model_hl_path, model_slowfast=model_slowfast_path, model_clip=model_clip_path)
    ...
    def make(self, u_id: str, query: str):
        ...
        # #///////////////////////////////
        # 		#----gen description----
        # 		des = self.hlw.get_description(self.gem, image)
        # 		descriptions[vid] = des
        # 		#///////////////////////
        
        #----get matching v_id----
        # new_descriptions = self.hlw.get_matching_description(self.openai, descriptions, self.n_scene)
    ...
    def make2(self, u_id: str, query: str):
        ...
        for i, (v_id, des) in enumerate(new_descriptions.items()):
            ...
            #     mute = False
            result = self.vew.split(u_id=u_id, start_time=max(vhighlight[v_id].start_time - self.duration_scene//2, 0), duration=self.duration_scene, video_input_path=vhighlight[v_id].path_video, output_path=mini_video_path, mute=mute)
            if result["success"]:
                list_mini_video.append(mini_video_path)
...
if __name__=="__main__":
    vm = VideoMaketing()
    u_id = "spa"
    vm.make2(u_id, "The services: massage, skincare, whitening bath, sauna")
