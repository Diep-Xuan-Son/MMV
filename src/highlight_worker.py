...
class HighlightWorker(object):
	def __init__(self, model_hl='./weights/model_highlight.ckpt', model_slowfast='./weights/SLOWFAST_8x8_R50.pkl', model_clip='./weights/ViT-B-32.pt'):
	...
	def get_description(self, llm: object, image: str, ):
		...
		prompt = ChatPromptTemplate.from_messages([
			...
			HumanMessage(content=[{"type":"text", "text":prompt.format(query="Give me a marketing video that introduces the MQ spa")}, {"type": "image_url", "image_url": {"url": f"data:image/jpeg;base64,{base64Image}"}}])
		])
	...
	def get_matching_description(self, llm: object, descriptions: dict, n_video: int):
		def OutputStructured(BaseModel):
		...
		chat_prompt = ChatPromptTemplate.from_messages(
			[
				...
				HumanMessage(content=prompt.format(query="Give me a marketing video that introduces the MQ spa", descriptions=descriptions, n_video=n_video))
